...
class DNC(snt.RNNCore):
  ...
  def __init__(self,
    ...
    with self._enter_variable_scope():
      ...
      self._controller = snt.DeepRNN([snt.Linear(controller_config['hidden_size'] + 3)], skip_connections=False)
  ...
  def _build(self, inputs, prev_state):
    ...
    controller_output_weights = controller_output[:,0:3]
    controller_output = controller_output[:,3:]
    ...
    with tf.name_scope('output_linear_fancy') as scope:
        weights = tf.nn.softmax(controller_output_weights)

        alu_1 = snt.Linear(3, name='alu_1')(output)
        alu_2 = snt.Linear(3, name='alu_2')(output)
        alu_3 = snt.Linear(3, name='alu_3')(output)

        weighted_alu_1 = tf.multiply(alu_1, tf.expand_dims(weights[:,0], axis=-1))
        weighted_alu_2 = tf.multiply(alu_2, tf.expand_dims(weights[:,1], axis=-1))
        weighted_alu_3 = tf.multiply(alu_3, tf.expand_dims(weights[:,2], axis=-1))
