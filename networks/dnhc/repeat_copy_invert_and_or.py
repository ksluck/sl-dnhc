...
class RepeatCopy(snt.AbstractModule):
  ...
  def _build(self):
    ...
    # In this version we also reserve one additional dimension for invert or not.
    full_obs_size = num_bits + 2 + 1
    # We reserve one target dimension for the end-marker.
    full_targ_size = num_bits + 1
    start_end_flag_idx = full_obs_size - 3
    num_repeats_channel_idx = full_obs_size - 2
    invert_channel_idx = full_obs_size - 1
    ...
    invert_batch = tf.random_uniform(
        [batch_size], minval=1, maxval=5, dtype=tf.int32)

    # Pads all the batches to have the same total sequence length.
    total_length_batch = sub_seq_length_batch * (num_repeats_batch + 1) + 4
    ...
    for batch_index in range(batch_size):
      ...
      invert = invert_batch[batch_index]
      ...
      pattern_sum = tf.reduce_sum(obs_pattern, axis=0)
      and_pattern =  pattern_sum // tf.cast(sub_seq_len, tf.float32)
      and_pattern = tf.tile(and_pattern, [sub_seq_len * num_reps])
      and_pattern = tf.reshape(and_pattern, targ_pattern_shape)
      or_pattern = tf.clip_by_value(pattern_sum, 0, 1)
      or_pattern = tf.tile(or_pattern, [sub_seq_len * num_reps])
      or_pattern = tf.reshape(or_pattern, targ_pattern_shape)
      targ_pattern = tf.cond(tf.equal(invert, tf.constant(1)), lambda : targ_pattern, lambda: 1-targ_pattern)
      targ_pattern = tf.cond(tf.equal(invert, tf.constant(3)), lambda: and_pattern, lambda: targ_pattern)
      targ_pattern = tf.cond(tf.equal(invert, tf.constant(4)), lambda: or_pattern, lambda: targ_pattern)

      # Expand the obs_pattern to have two extra channels for flags.
      # Concatenate start flag and num_reps flag to the sequence.
      obs_flag_channel_pad = tf.zeros([sub_seq_len, 3])
      ...
      num_reps_flag = tf.one_hot(
          ...
          off_value=0.)
      invert_flag = tf.one_hot(
          [invert_channel_idx],
          full_obs_size,
          on_value= tf.cast(invert, tf.float32),
          off_value=0.)
      ...
      obs = tf.concat([obs, invert_flag], 0)
      ...
      targ_start_pad = tf.zeros([sub_seq_len + 3, full_targ_size])

      # The mask is zero during the obs and one during the targ.
      mask_off = tf.zeros([sub_seq_len + 3])
  ...
  def to_human_readable(self, data, model_output=None, whole_batch=False):
    ...
    unnormalised_num_reps_flag = self._unnormalise(obs[:,:,-2:-1]).round()
    obs = np.concatenate([obs[:,:,:-2], unnormalised_num_reps_flag], axis=2)
    obs = np.concatenate([obs, data.observations[:,:,-1:]], axis=2)
