...
class RepeatCopy(snt.AbstractModule):
  ...
  def _build(self):
    ...
    for batch_index in range(batch_size):
      ...
      targ_pattern_rotate = tf.concat([targ_pattern[:,1:], tf.expand_dims(targ_pattern[:,0], axis=1)], axis=1)
      targ_pattern_shift = tf.concat([targ_pattern[:, 1:], tf.zeros([sub_seq_len * num_reps, 1])], axis=1)
      targ_pattern_0 = tf.cond(tf.equal(invert, tf.constant(1)), lambda : targ_pattern, lambda: 1-targ_pattern)
      targ_pattern_1 = tf.cond(tf.equal(invert, tf.constant(3)), lambda: targ_pattern_rotate, lambda: targ_pattern_0)
      targ_pattern = tf.cond(tf.equal(invert, tf.constant(4)), lambda: targ_pattern_shift, lambda: targ_pattern_1)
